# coding=utf-8
# Copied from  https://github.com/google-research/google-research/blob/master/vct/src/elic.py
...
class ElicAnalysis(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if len(channels) not in (3, 4):
      raise ValueError(f"ELIC uses 3 or 4 conv layers (not {channels}).")
    ...
    self._downsample_factor = 2 ** len(channels)

    self._output_depth = channels[-1]

    # Keep activation separate from conv layer for clarity and symmetry.
    conv = functools.partial(
      build_conv, act=None, up_or_down="down")
    convs = [conv(output_channels=c, kernel_size=k, strides=s) for (c, k, s) in zip(channels,
                                                                                    kernel_sizes,
                                                                                    strides)]
  ...
  def compute_output_shape(self, input_shape):
    ...
    h, w = shape[-3], shape[-2]
    shape[-3:] = [h // self._downsample_factor, w // self._downsample_factor, self.output_depth]
...
class ElicSynthesis(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if output_channels is not None and output_channels != channels[-1]:
      raise ValueError("output_channels specified but does not match channels: "
                       f"{output_channels} vs. {channels}")
    self._upsample_factor = 2 ** len(channels)
    ...
    conv = functools.partial(
      build_conv, act=None, up_or_down="up")
    convs = [conv(output_channels=c, kernel_size=k, strides=s) for (c, k, s) in zip(channels,
                                                                                    kernel_sizes,
                                                                                    strides)]
  ...
  def compute_output_shape(self, input_shape):
    shape = tf.TensorShape(input_shape).as_list()
    h, w = shape[-3], shape[-2]
    shape[-3:] = [h * self._upsample_factor, w * self._upsample_factor, self.output_depth]
    return tf.TensorShape(shape)
